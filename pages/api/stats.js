...
export default async function handler(req, res) {
  try {
    ...
    const sessions = await db.collection('sessions')
      .find({ userId, status: { $ne: 'active' }, type: { $ne: 'mobility' } }).sort({ completedAt: -1 }).limit(50).toArray();
  ...
  const volumeByDay = { brust: [], ruecken: [], beine: [], kettlebell: [], fullbody: [], fullA: [], fullB: [], push: [], pull: [], legs: [] };
  sessions.forEach(s => {
